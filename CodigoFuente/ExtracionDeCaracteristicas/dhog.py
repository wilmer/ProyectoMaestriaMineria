	...
	for h,row in enumerate(image_array):
		for w, val in enumerate(row):
			if h-1>=0 and w-1>=0 and h+1<max_h and w+1<max_w:
				...
				grad[h][w] = np.arctan(dy/dx)*(180.0/np.pi)
				if grad[h][w]<0:
					grad[h][w] += 180.0
...
if __name__ == '__main__':
	create_hog_file('data_test/frame135.jpg','data_test_out/frame135.jpg.json')
	create_hog_file('data_test/frame136.jpg','data_test_out/frame136.jpg.json')
	create_hog_file('data_test/frame137.jpg','data_test_out/frame137.jpg.json')
	create_hog_file('data_test/frame138.jpg','data_test_out/frame138.jpg.json')
	##create_hog_file('data/cat.jpg','cat.txt')
	##mg = read_hog_file('cat.txt')
	##print(mg)
	##print(mg.shape)

	##mdir = 'D:/WILMER/UNSA/MAESTRIA/Datamining/ExtracionDeCaracteristicas/data_test'
	##j_out='D:/WILMER/UNSA/MAESTRIA/Datamining/ExtracionDeCaracteristicas/data_test_out'
	##folders = os.listdir(mdir)
	#path = 'D:/WILMER/UNSA/MAESTRIA/Datamining/ExtracionDeCaracteristicas/data_test/'
	##pathout = 'D:/WILMER/UNSA/MAESTRIA/Datamining/ExtracionDeCaracteristicas/data_test_out/'		
	##mdir = 'D:/WILMER/UNSA/MAESTRIA/Datamining/ExtracionDeCaracteristicas/data_test/'
	##folders = os.listdir(mdir)
